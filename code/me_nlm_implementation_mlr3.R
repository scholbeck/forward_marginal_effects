...
testEnvelopeCateg = function(train, test, level) {
  ...
  extrap_preds_test = extrap_risk_mod$predict_newdata(combined.df)$prob[, 1]
    # predict(extrap_risk_mod, newdata = combined.df)
...
marginalEffect = function(feature, step.size, data, model) {
  prediction.before = model$predict_newdata(data)$prob[, 1]
    # predict(model, newdata = data)
  data.intervened = as.data.frame(data)
  data.intervened[ , feature] = data.intervened[ , feature] + as.numeric(step.size)
  prediction.after = model$predict_newdata(data.intervened)$prob[, 1]
    # predict(model, newdata = data.intervened)
...
marginalEffectCateg = function(feature, level, data, model) {
  prediction.before = model$predict_newdata(data[data[, feature] != level, ])$prob[, 1]
    # predict(model, newdata = data[data[, feature] != level, ])
...
predFunctionMinusSecantSquared = function(model, observation, feature, step_size, t) {
  ...
  model_prediction = model$predict_newdata(modified_obs)$prob[, 1]
    # predict(model, newdata = modified_obs) # f(x(t))
...
predMean = function(model, observation, feature, step_size) {
  fn = function(t) {
    modified_obs = parametrizedFeatures(observation, feature, step_size, t) # x(t)
    model_predictions = model$predict_newdata(modified_obs)$prob[, 1]
      # predict(model, newdata = modified_obs) # f(x(t))
...
secant = function(observation, step_size, feature, model, m) {
  target_start = model$predict_newdata(observation)$prob[, 1]
    # predict(model, newdata = observation)
