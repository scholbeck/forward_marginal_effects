...
source("paper/code/partition_tree.R")
library(partykit)
library(parttree)
library(ggplot2)
...
nlm = nonLinearityMeasureDataFrameDataGenProcess(dataGeneratingProcessMultiple, df, feature = c('x1', 'x2'), step_size = c(2, 3))
tree = rpart(me ~ x1 + x2, data = df)
me_pred = predict(tree, df)
df$nlm = nlm
